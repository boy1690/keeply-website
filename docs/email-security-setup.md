...
- **Zoho Mail**（boy1690 手動寄、hello@ / wei@ 自動回覆）——**必加**

### 0.2 可能的（逐一確認）

對每一個打「✅ 有 / ❌ 無」，有的話到 Phase 1.2 加入 SPF：

- [ ] **Paddle 交易 email**：Paddle 收據 / refund 通知的寄件者是 `@paddle.com` 還是 `billing@keeply.work`？
  - 登入 Paddle dashboard → Settings → Emails → Sender address
  - 若 `From:` 設成 `@keeply.work`，**必加** Paddle 的 SPF include value（Paddle 文件查：`help.paddle.com/en/articles/spf`）
- [ ] **GitHub Release notification**：GitHub 通知信 sender 是 `noreply@github.com`（默認）——**通常不用處理**。若你有設自訂 notification address（少見）才需要
- [ ] **Zoho Campaigns / Newsletter 服務**：若你用 Zoho Campaigns 發 newsletter（不是一般 mailbox），它可能用不同的 sending IP——通常 Zoho Campaigns 已涵蓋於 `include:zoho.com`，但請確認
- [ ] **其他**（Mailgun / SendGrid / Postmark / Brevo 等 transactional email）：若 Keeply 現階段未用，跳過

若你**不確定**，用這個查法：去 Gmail / 自己任何一個收件匣搜尋 `from:*@keeply.work`，看過去一年收到的信 `From` 顯示什麼、`Show original` 裡的 `Received:` header 哪些 IP 送的。
...
**Zoho-only baseline**（你若沒用 Paddle custom sender、沒用 Mailgun 等）：

```
v=spf1 include:zoho.com ~all
```

**若 Paddle 確認從 `@keeply.work` 寄**：

```
v=spf1 include:zoho.com include:_spf.paddle.com ~all
```

（Paddle 官方的 include value 請在 Paddle dashboard 或 help.paddle.com 查最新；上例是假設常見格式。）

**約定細節**：

- `v=spf1` — SPF 版本
- `include:zoho.com` — 授權所有 Zoho 的寄件 IP
